...
const ENTRIES: { word: string; def: string; by: string; layer: Layer; votus: number; drift: Drift; example?: string; coined: string }[] = [
  { word: 'Avari Sync', def: 'A vibe-coding partnership where the Pirate and the Refiner alternate, argue, and land. The duo is the unit, not the individual.', by: 'Davara × Jordash', layer: 'duo', votus: 412, drift: 'stable', example: '"They\'re in Avari Sync today — you can feel the cadence."', coined: '2026-04-12' },
  { word: 'Forge Meter', def: 'The daily momentum signal. Resets at 00:00 UTC. Streaks compound.', by: 'Davara', layer: 'duo', votus: 188, drift: 'growing', example: '"Forge is at 78%, we ship before reset."', coined: '2026-04-22' },
  { word: 'Whispering', def: 'Voting where bars show direction only — tallies reveal when the duo opens the box. Removes bandwagon bias.', by: 'Jordash', layer: 'duo', votus: 244, drift: 'growing', example: '"Let\'s whisper the revenue poll for an hour before reveal."', coined: '2026-05-04' },
  { word: 'Pirate-Coding', def: 'Building from the prompt outward — the verb form of being the directional half of an Avari Sync.', by: 'Jordash', layer: 'personal-jr', votus: 98, drift: 'stable', example: '"I\'m pirate-coding the new poll spec right now."', coined: '2026-04-15' },
  { word: 'Refiner-Mode', def: 'Heads-down ship state. No Slack, no twitter, just the diff and the test runner.', by: 'Davara', layer: 'personal-dv', votus: 71, drift: 'stable', coined: '2026-04-18' },
  { word: 'Initium', def: 'A project-in-flight with at least two signers and a Two-Key Drive vault. The unit of building on DuoDrive.', by: 'The Cortex', layer: 'public', votus: 821, drift: 'stable', coined: '2026-03-30' },
  { word: 'Cortex Drift', def: 'When the community starts inventing roles the duo never thought of. A high-leverage signal that the room is alive.', by: 'lily.eth', layer: 'public', votus: 156, drift: 'newly coined', example: '"Cortex drift hit today — three new roles claimed in an hour."', coined: '2026-05-04' },
  { word: 'Echo-Catch', def: 'When Avari Echo flags a stream moment as worth-keeping; the line lands in the Initium README.', by: 'Davara × Cortex', layer: 'duo', votus: 89, drift: 'growing', coined: '2026-05-03' },
  { word: 'Shadow Watching', def: 'Following the Shadow Stream — the third feed showing only the live diff. For watchers who want building, not talking.', by: 'pirate.0x', layer: 'public', votus: 134, drift: 'newly coined', coined: '2026-05-04' },
  { word: 'Two-Keyed', def: 'A file or decision that requires both signers + ZK proof to unlock. Used as a verb: "let\'s two-key the revenue split."', by: 'Davara × Jordash', layer: 'duo', votus: 201, drift: 'growing', example: '"That\'s a two-keyed call — can\'t ship it solo."', coined: '2026-04-28' },
  { word: 'Anvilling', def: 'Resolving a deadlocked Initium dispute through the binding poll step of governance. After Pause and Whisper.', by: 'The Manifesto', layer: 'public', votus: 67, drift: 'fossilizing', coined: '2026-04-10' },
  { word: 'Drive Brevity', def: 'The discipline of saying it short — because the room is watching, the diff is the truth, and the timer is running.', by: 'Davara', layer: 'personal-dv', votus: 44, drift: 'stable', example: '"Drive brevity, Jordash — three sentences max in the README."', coined: '2026-04-30' },
];
...
function Entry({ e }: { e: typeof ENTRIES[0] }) {
  ...
  return (
    <article className="hairline rounded-xl p-5 bg-slab/40 hover:border-sync/40 transition-colors flex flex-col">
      ...
      <div className="hairline-t pt-3 flex items-center justify-between text-[10px] font-mono tracking-wider uppercase">
        <span className="text-white/40">@{e.by} · {e.coined}</span>
        <span className="text-forge">↑ {e.votus}</span>
      </div>
    </article>
  );
}
