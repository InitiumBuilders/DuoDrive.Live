  ...
  return (
    <div className="px-5 md:px-8 max-w-6xl mx-auto pt-28 pb-20">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {CODERS.map((c) => (
          <div key={c.handle} className="hairline rounded-xl p-5 glass group hover:border-sync/40 transition-colors">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-mono tracking-wider uppercase text-fg-muted">Drive {c.drive}</p>
                </div>
              </div>
              {c.status === 'Live now' && (
            ...
            <div className="flex flex-wrap gap-1.5 mb-4">
              {c.vibes.map((v) => (
                <span key={v} className="text-[10px] font-mono tracking-wider uppercase text-fg-muted hairline rounded px-2 py-0.5">{v}</span>
              ))}
            </div>
            <div className="hairline-t pt-3 flex items-center justify-between">
              <span className="text-[11px] text-fg-muted">{c.status}</span>
